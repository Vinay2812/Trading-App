...
import genereateOTP from "../EmailVerification/OTP.js";
import sendEMail from "../EmailVerification/email.js"
...
export async function register(req, res) {
    ...
    try {
        ...
        const bankDetails = await Promise.all(bankData.map(async (data) => {
            ...
            return await(await executeQuery(BANK_DATA_QUERY))[0];
        }));

        const contactDetails = await Promise.all(contactData.map(async (data) => {
            ...
            return await(await executeQuery(CONTACT_QUERY))[0];
        }));
        ...
        res.status(200).json({userData: userDetails, bankData: bankDetails, contactData: contactDetails});
